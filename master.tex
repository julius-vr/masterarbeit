...
\end{align*}
for each simplex $\sigma \subset X$, since either $H_{|\sigma \times \{ s \}}=id$ for all $s$ (namely if $\sigma$ is contained in $X-(int(B-C))$ or if $\sigma$ is already in general position with respect to $A$), or $\sigma \subset B-C$ is moved by a local shift, in which case we have $dim(H(\sigma \times I))=dim(\sigma)+1$.
\end{proof}

BEWEIS DARUEBER: ARGUMENTE

\section{Intersection bordism}
 
\begin{definition}
For a PL space $P$, a PL map $f: P \to X$ is called \textbf{($\boldsymbol{\overline{p}}$,i)-allowable} if 
\begin{align*}
...
\end{align*}
by sending $[g]$ to the corresponding class in $I\Omega_i^{\mathcal{L}, \overline{p}}(X)$ that is represented by $g$.
